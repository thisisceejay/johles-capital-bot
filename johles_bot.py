...
def fmt_week_view(events):
    ...
    for e in filtered:
        t   = parse_event_time(e)
        day = t.strftime("%a %d %b").upper() if t else "TBC"
        days.setdefault(day, []).append(e)
    msg = f"\U0001f4c5 <b>WEEK  \u258c{len(filtered)} HIGH-IMPACT EVENTS</b>\n\n"
    for day, day_events in days.items():
        ...
        for e in day_events:
            ...
            forecast = e.get("forecast", "\u2014") or "\u2014"
            prev     = e.get("previous", "\u2014") or "\u2014"
            msg += f"  \U0001f534 <b>{ts}</b>  {e.get('country','').upper()}  {e.get('title','')}\n"
            msg += f"      F <code>{forecast}</code>  P <code>{prev}</code>\n"
        msg += "\n"
    return msg.rstrip()
...
def fmt_today_view(events):
    ...
    if not filtered:
        return f"\U0001f4c5 <b>{today.strftime('%a %d %b').upper()}</b>  \u2705 Clear \u2014 no high-impact events today."
    msg = f"\U0001f4c5 <b>{today.strftime('%a %d %b').upper()}  \u258c{len(filtered)} EVENT{'S' if len(filtered)>1 else ''}</b>\n\n"
    for e in filtered:
        t        = parse_event_time(e)
        ts       = t.strftime("%H:%M") if t else "TBC"
        forecast = e.get("forecast", "\u2014") or "\u2014"
        prev     = e.get("previous", "\u2014") or "\u2014"
        diff     = int((t - now).total_seconds() / 60) if t else 0
        eta      = f"+{abs(diff)}min ago" if diff < 0 else f"in {diff}min"
        msg += f"\U0001f534 <b>{ts}</b>  {e.get('country','').upper()}  {e.get('title','')}  <i>{eta}</i>\n"
        msg += f"    F <code>{forecast}</code>  P <code>{prev}</code>\n\n"
    return msg.rstrip()
...
def fmt_next_event(events):
    now      = datetime.utcnow()
    upcoming = sorted([(parse_event_time(e), e) for e in filter_events(events) if parse_event_time(e) and parse_event_time(e) > now], key=lambda x: x[0])
    if not upcoming:
        return "\u23ed <b>NEXT</b>  No upcoming high-impact events this week."
    t, e     = upcoming[0]
    ...
    return (
        f"\u23ed <b>NEXT  \u258c{eta} away</b>\n"
        f"\U0001f534 <b>{e.get('country','').upper()}  {t.strftime('%H:%M')} UTC  {t.strftime('%a %d %b').upper()}</b>\n"
        f"{e.get('title','')}\n"
        f"<code>{pairs}</code>\n"
        f"F <code>{forecast}</code>  P <code>{prev}</code>"
    )
...
def fmt_help():
    return (
        ...
        "/help  \u2014 This menu\n\n"
        "<i>Auto: session open \u00b7 30min \u00b7 10min \u00b7 result</i>\n"
        "<i>EURUSD \u00b7 GBPUSD \u00b7 USDJPY \u00b7 AUDUSD \u00b7 USDCAD \u00b7 NZDUSD \u00b7 USDCHF \u00b7 XAUUSD</i>"
    )
